...
<tr>
	<td valign="top">
		...
		<!--------------- 탭패널 시작 --------------------->
			...
			, 'shople/index.php'
			, 'selly/index.php'
			, 'goods/adm_goods_list.php' //상품V2
			, 'board/list_management.php' //게시판(통합관리)
			, 'overseas/intro.php' // 해외구매대행
			);
			$tmp = array();
			if(trim($_SERVER['SCRIPT_FILENAME']) != ''){
				$panelTabMenuUrlArray = array();
				$panelTabMenuUrlArray = array_reverse(explode('/', $_SERVER['SCRIPT_FILENAME']));
				reset($panelTabMenuUrlArray);
				$tmp[0] = next($panelTabMenuUrlArray).'/'.prev($panelTabMenuUrlArray);
			}
