<?php /* Template_ 2.2.7 2015/12/28 15:21:51 /www/hummingimc_godo_co_kr/shop/data/skin/standard/mypage/mypage.htm 000017215 */ 
...
<?php if($TPL_loop_1){foreach($TPL_VAR["loop"] as $TPL_V1){?>
		var tmp = "<div class='slidediv'><a href='<?php echo $TPL_V1["goods_view_url"]?>'><?php echo goodsimg($TPL_V1["img_s"],$TPL_VAR["size"],addslashes('class='.$TPL_VAR["css_selector"].' rsize'))?></a></div><div class='stitle'><?php if($TPL_V1["coupon"]){?><div><span class='pink bold'><?php echo $TPL_V1["coupon"]?></span> <img src='/shop/data/skin/standard/img/icon/good_icon_coupon.gif' align='absmiddle'></div> <?php }?><?php if($TPL_V1["brandnm"]){?><div class='stxt'><span class='bold'><?php echo $TPL_V1["brandnm"]?></span></div> <?php }?><?php if($TPL_V1["goodsnm"]){?><div class='gsnm'><a href='<?php echo $TPL_V1["goods_view_url"]?>' title='<?php echo strip_tags($TPL_V1["goodsnm"])?>'><?php echo strip_tags($TPL_V1["goodsnm"])?></a> </div><?php }?><?php if($TPL_V1["shortdesc"]){?><div class='stxt'><?php echo nl2br($TPL_V1["shortdesc"])?> </div><?php }?><?php if($TPL_V1["soldout_icon"]){?><div><?php if($TPL_V1["soldout_icon"]=='custom'){?><img src='../data/goods/icon/custom/soldout_icon'><?php }else{?><img src='/shop/data/skin/standard/img/icon/good_icon_soldout.gif'><?php }?></div><?php }?><?php if($TPL_V1["icon"]){?><?php echo $TPL_V1["icon"]?> <?php }?></div>\
		<div class='sprice'><?php if(!$TPL_V1["strprice"]){?><?php if($TPL_V1["goodsDiscountPrice"]){?><?php if($TPL_V1["oriPrice"]){?><div class='gray'><strike><?php echo number_format($TPL_V1["oriPrice"])?></strike>↓</div><?php }?><span><?php echo number_format($TPL_V1["goodsDiscountPrice"])?>원</span><?php }elseif($TPL_V1["price"]){?><?php if($TPL_V1["consumer"]){?><div class='gray'><strike><?php echo number_format($TPL_V1["consumer"])?></strike>↓</div><?php }?><span><?php echo number_format($TPL_V1["price"])?>원</span> <?php if($TPL_V1["special_discount_amount"]){?><img src='/shop/data/skin/standard/img/icon/goods_special_discount.gif'><?php }?><?php }?><?php if($TPL_V1["soldout_price_string"]){?><?php echo $TPL_V1["soldout_price_string"]?><?php }?><?php if($TPL_V1["soldout_price_image"]){?><?php echo $TPL_V1["soldout_price_image"]?><?php }?><?php }else{?><?php echo $TPL_V1["strprice"]?><?php }?></div>\
...
<div class='mypagediv'>
	<!-- 상단이미지 || 현재위치 -->
	<div>
		<div><img src='/shop/data/skin/standard/img/common/title_mypage.gif' border=0></div>
...
<?php if($TPL_V1["step"]== 3&&!$TPL_V1["step2"]){?>
										<a href='javascript:order_confirm(<?php echo $TPL_V1["ordno"]?>)'><img src='/shop/data/skin/standard/img/common/btn_receive.gif'></a>
<?php }elseif($TPL_V1["escrowconfirm"]== 2){?>
										수령
<?php }?>
								</td>
								<td><a href='<?php echo url("mypage/mypage_orderview.php?")?>&ordno=<?php echo $TPL_V1["ordno"]?>'><img src='/shop/data/skin/standard/img/common/btn_detailview.gif'></a></td>
							</tr>
<?php }}?>
<?php }else{?>
						<tr>
...
<?php if($TPL_V1["m_no"]==$GLOBALS["sess"]["m_no"]){?>
								<a href='javascript:popup_register( "reply_qna", "<?php echo $TPL_V1["sno"]?>" );'><img src='/shop/data/skin/standard/img/common/btn_reply.gif' border='0' align='absmiddle'></a>
								<a href='javascript:popup_register( "mod_qna", "<?php echo $TPL_V1["sno"]?>" );'><img src='/shop/data/skin/standard/img/common/btn_modify2.gif' border='0' align='absmiddle'></a>
								<a href='javascript:popup_register( "del_qna", "<?php echo $TPL_V1["sno"]?>" );'><img src='/shop/data/skin/standard/img/common/btn_delete.gif' border='0' align='absmiddle'></a>
<?php }?>
							</td>
						</tr>
<?php }}?>
	...
	<div class='mplist'>
		...
		<div id='latelyList' class='ordlistdiv'>
			<table cellpadding='0' cellspacing='0' border='0' class='slidetbl'>
				<tr align='center'>
					<td class='prev'>
						<div class='slidebtn'><a href='javascript:latelySlide.go()'><img src='/shop/data/skin/standard/img/common/btn_common_prev.gif' onmouseover='latelySlide.direct(-1)'></a></div>
					</td>
					<td valign='top'>
						<script>latelySlide.exec();</script>
					</td>
					<td class='next'>
						<div class='slidebtn'><a href='javascript:latelySlide.go()'><img src='/shop/data/skin/standard/img/common/btn_common_next.gif' onmouseover='latelySlide.direct(1)'></a></div>
					</td>
				</tr>
			</table>
		</div>
